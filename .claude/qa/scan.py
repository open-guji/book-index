...
各項檢查（字母即報表之鍵）：
  ...
  K entity_oneway    work.authors.entity_id 有繫而 entity.works 無回指；entity.works 指向不存在之 work
  L lone_outlier     一 entity 名下諸 work 之 period，n−1 同而恰一異（磁鐵／誤繫徵候）
  M index_drift      索引 period/loss_status/title/subtype 與記錄檔不符
  N source_no_bid    indexed_by 有 source 而無 source_bid
...
def run_checks(works, IW, IB, IE, IC, ents):
    ...
    for w in works.values():
        ...
        for ib in (w.get('indexed_by') or []) + (w.get('emendated_by') or []):
            ...
            if ib.get('source') and not ib.get('source_bid'): R['N'].append(row(w, source=ib.get('source')))
        # C / F / K(oneway)
        for a in a_list:
            eid = a.get('entity_id')
